...

{
	...
	//function declare
	float Brandlist (char);
	int inputoptproduct (int);
	void getcustinfo(string ,string ,string ,string ,string ,string ,string ,string ,string );
	...
	cout << "\n WELCOME TO WALMART WAREHOUSE IMPORT/EXPORT SYSTEM" <<endl;
	...
	while (toupper (choice) != 'N')
	{
		...
		{	
			...
			codebrand = Brandlist (codebrand);
		
			while (codebrand != 'A' && codebrand != 'B' && codebrand != 'C' && codebrand != 'D' && codebrand != 'E' &&  codebrand != 'F' && codebrand != 'G' )
			{
		...
		getcustinfo(name, email, premistype,storefront,street,city,state,phonenum,postcode);
...
// FUNCTION
	...
	void optMunchyList()
	{
			cout << "\n OPTION | TYPE OF PRODUCT AVAILABLE | Item/carton |\tPrice/carton" <<endl;
			cout << "--------------------------------------------------------------------"<<endl;
	...
	void optOLDTOWNList()
	{
			...
			cout << "  3 \t White Coffee 3in1 Less Sugar 15*38g \t\t 12 \t\t RM 185.9"<<endl;
...
//Brahims
	...
	void getcustinfo(string name,string email,string premistype,string storefront,string street,string city,string state,string phonenum,string postcode)
	{
		cout<<"\t" <<name<<endl;
		cout<<"\t" <<phonenum<<endl;
		cout<<"\t" <<email<<endl;
